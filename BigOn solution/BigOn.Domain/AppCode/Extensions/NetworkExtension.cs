    ...
    public  static partial class Extension
    {
        static public bool SendMail( this IConfiguration configuration, string toEmail, string textBody ,string textSubject )
        {
            try
            {
                ...
                SmtpClient client = new SmtpClient(configuration["emailAccount:smtpServer"], Convert.ToInt32(configuration["emailAccount:smtpPort"]));
                client.Credentials = new NetworkCredential(configuration["emailAccount:userName"], configuration["emailAccount:password"]);
                client.EnableSsl = true;

                MailAddress from = new MailAddress(configuration["emailAccount:userName"], configuration["emailAccount:displayName"]);
